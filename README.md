...
This project implements a professional-grade **Continuous Integration and Continuous Deployment (CI/CD)** workflow using **GitHub Actions**. Every code change undergoes a rigorous validation process to ensure stability and security.

### 🛠️ Pipeline Architecture
The workflow is divided into three sequential stages:

1.  **Automated Testing**: 
    - Executes **JUnit** unit tests on every push.
    - Generates and uploads detailed HTML test reports as artifacts for quality auditing.
2.  **Secure Build Process**: 
    - Compiles the Android application using **Gradle**.
    - **Security First**: Injects the `GEMINI_API_KEY` via GitHub Secrets, ensuring no sensitive credentials are ever exposed in the source code.
    - Optimized with **R8/ProGuard** for code obfuscation and resource shrinking (reducing APK size and protecting intellectual property).
3.  **Automated Deployment (Releases)**:
    - Upon successful testing and building, the pipeline automatically creates a **GitHub Release**.
    - Attaches the verified **APK binary** for immediate testing and distribution.

### 🧠 Intelligent Features
- **Gemini 1.5 Flash**: Leveraged for multimodal analysis (Image-to-Text) to provide expert breed descriptions.
- **On-Device ML**: Integration with **TensorFlow Lite** and **ML Kit** for real-time dog breed recognition.

### 📦 How to get the latest APK
You can always find the latest, verified version of the app in the [Releases] section of this repository. Each build is tagged and documented automatically.
